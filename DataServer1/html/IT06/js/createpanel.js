 ...
 * Copyright (c) 2020
 *
 * This file contains all of the necessary functionality for creating panels
 * on the create panel page of the web interface.
 *
 * @summary Functionality for create panel page on the web interface
 ...
 (function($) {
    ...
    function addStream() {
        ...
        $("#inputDatatype_" + streams).on("change", function() {
            ...
            var combosin = form.querySelectorAll("#inpute_" + streams).length;
            ...
            $("#inputFunction_" + streams).on("change", function() {
                ...
                if (func == "sin") { //if sin selected
                    addSin(streams, form, rand, sin, combosin);
                } else if (func == "random") { //if random selected
                    addRandom(streams, form, rand, sin, combosin);
                } else if (func == "sinCombo") { //if sincombo selected
     ...
     * @param {array} streams 
     */
    function saveStreams(username, panelname, streams) {
        //create object containing panel information
        var data = {
            username: username,
            panelname: panelname,
            streams: streams
        ...
        fetch("../php/createPanel.php", {
            ...
            .then((data) => {
                if (data == "panel created successfully") { //if successfull
                    alert(data);
    ...
    async function saveData(username, panel_id) {
        ...
        saveStreams(username, panelname, streams);
    }
    ...
    $("#createForm").on("submit", function(event) {
        ...
        //get username of logged in
        var username = getCookie("username");

        //validate form data
        var valid = validate();

        //if form data valid, submit
        if (valid) {
            saveData(username);
        }
    });
